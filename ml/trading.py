...
class TradingEnv(gym.Env):
    ...
    def __init__(self, ob_file,lvls_obs=LVLS_IN_OBS):
        column_names = ['price', 'quantity', 'side'] 
        self.ob = pd.read_csv(ob_file, names=column_names)
        self.norm_fac_price=48000.   
    ...
    def step(self, action):
        ...
        if buy:
            ...
            self.balance-=sell_price*action
        ...
        self.t+=1
        done = self.t*100>=len(self.ob) and self.balance+self.quantity*sell_price>0 
        if(done): 
